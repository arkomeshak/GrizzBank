...
class Username(models.Model):
    username = models.fields.CharField(max_length=45, primary_key=True)
    client = models.ForeignKey(Client, on_delete=models.CASCADE, primary_key=True)
    client_id = models.fields.IntegerField(primary_key=True)


class PhoneNumber(models.Model):
    client = models.ForeignKey(Client, on_delete=models.CASCADE, primary_key=True)
    phone_number = models.fields.CharField(max_length=10, primary_key=True)


class InterestRate(models.Model):
    acct_type = models.fields.CharField(max_length=1, primary_key=True)
    interest_rate = models.fields.DecimalField(default=0.0)


class Email(models.Model):
    client = models.ForeignKey(Client, on_delete=models.CASCADE, primary_key=True)
    email = models.fields.CharField(max_length=50, primary_key=True)


class Account(models.Model):
    acct_id = models.fields.AutoField(primary_key=True)
    acct_bal = models.fields.DecimalField(max_digits=5, decimal_places=2, null=False)
